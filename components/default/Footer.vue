    ...
    <div class="container mx-auto pt-6 px-4">
      <div class="flex-col flex md:flex-row md:items-center md:space-x-12">
        ...
        <div class="flex flex-col items-start w-full space-y-4">
          <h3 class="text-secondary text-lg">Get In Touch</h3>
          <ul
            class="flex-col flex space-y-3 md:flex-row md:items-center md:space-x-8 md:space-y-0"
          >
            <li
      ...
      <div
        class="flex-col flex text-white my-8 space-y-6 md:space-y-0 md:my-12 md:flex-row"
      >
        <div class="flex flex-col justify-between flex-grow md:pr-8 md:w-1/4">
          <div v-if="footer" class="flex flex-col space-y-3 md:space-y-6">
          ...
          <div
            class="flex mt-4 flex-col space-x-0 items-start space-y-3 xl:flex-row xl:items-center xl:space-x-8"
          >
        ...
        <div
          class="flex-col flex items-start justify-evenly space-y-4 md:space-y-0 md:flex-row md:border-r md:border-l md:w-1/2"
        >
        ...
        <div class="flex flex-col justify-between md:pl-8 md:w-1/4">
          <div>
            <h4 class="mb-3 md:mb-6 text-lg font-semibold">Country Settings</h4>
            <div v-if="defaultCountry" class="flex items-center space-x-2">
              <Icon
          ...
          <div class="mt-4">
            <h4 class="font-semibold">Shop with confidence</h4>
          </div>
        </div>
      </div>
      <hr class="my-2 border-dashed border-gray-200 mx-auto" />
      <ul
        class="text-white flex flex-wrap items-center justify-center space-x-6 leading-10 mb-2 md:space-x-12"
      >
        <li
...
const links = computed(() => {
  const footerLinks = []
  if (props.menus?.categories) footerLinks.push(props.menus.categories)

  if (props.menus?.useful_links) footerLinks.push(props.menus.useful_links)

  if (props.menus?.help_center) footerLinks.push(props.menus.help_center)

  return footerLinks
})
const defaultCountry = computed(() => {
  if (props.menus?.countries)
    return props.menus.countries.items.forEach((c) =>
