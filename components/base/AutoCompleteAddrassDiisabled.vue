...
watch(shop, (shopData) => {
  ...
  if (!streetAddress2.value)
    streetAddress2.value =
      shopData.prefillData[props.baseClass].address_line_two
  if (!postCode.value)
    postCode.value = shopData.prefillData[props.baseClass].post_code
  if (!city.value) city.value = shopData.prefillData[props.baseClass].city
  if (!state.value) state.value = shopData.prefillData[props.baseClass].state
  if (!country.value)
...
onMounted(() => {
  ...
  initializeAutocomplete(
    document.querySelector(`.${props.baseClass} .street_address`),
    'street_address'
  )
  ...
  if (!city.value) city.value = shop.prefillData[props.baseClass].city
  if (!state.value) state.value = shop.prefillData[props.baseClass].state
  if (!country.value) country.value = shop.prefillData[props.baseClass].country
...
const handleCountryCode = (value) => {
  if (props.baseClass === 'shipping') {
    ...
    if (code) useShopStore().getShippingDetails(code)
    else useShopStore().clearShippingDetails()
  }
}
...
const initializeAutocomplete = (element, type) => {
  ...
  autocomplete.addListener('place_changed', () => {
    ...
    address.value = place
    if (place.formatted_address) updateField(place, place.address_components)
  })
}

const updateField = (place, addressComponents) => {
  const countryObj = addressComponents.filter((item) =>
    item.types.includes('country')
  )[0]
  const stateObj = addressComponents.filter((item) =>
    item.types.includes('administrative_area_level_1')
  )[0]
  const postalObj = addressComponents.filter((item) =>
    item.types.includes('postal_code')
  )[0]
  const cityObj = addressComponents.filter((item) =>
